...
for (i in 1:length(input_files)){
  ...
  if (width(readDNAStringSet(input_files[i])[1]) != 0) {
...
prepare_data <- function(dat){
  for (idx in 1:length(dat)) {
    ...
    x <- names(dat[[idx]])
    #base_tab$sample_name <- strsplit(names(dat[[idx]]), "_")[[1]][2]
    ...
    base_tab$freq <- as.numeric(base_tab$freq)
    ...
    base_dat <- data.frame(sample=sapply(strsplit(x, "_"), function(x) paste(x[1], collapse = "-")), position=1:length(base_seq))
